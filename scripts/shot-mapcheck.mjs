...
await page.waitForSelector("canvas",{timeout:30000}); await page.waitForTimeout(3500);
await page.screenshot({ path: "/tmp/cm-mapcheck.png" });
// count canvases (maplibre basemap + deck overlay)
const canvases = await page.evaluate(()=>document.querySelectorAll("canvas").length);
const mapTiles = await page.evaluate(()=>document.querySelectorAll(".maplibregl-canvas").length);
console.log("canvases:", canvases, "maplibre canvas:", mapTiles, "ERRORS", JSON.stringify(errors.slice(0,6)));
await browser.close();
